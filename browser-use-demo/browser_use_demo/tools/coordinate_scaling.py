...
class CoordinateScaler:
    ...
    def get_documented_size_for_aspect_ratio(
        ...
        supported_ratios = [
            f"{w}:{h} ({w / h:.3f})" for (w, h) in cls.DOCUMENTED_SIZES.keys()
        ]
        raise ValueError(
    ...
    def get_scale_factors(
        cls, viewport_width: int, viewport_height: int, match_aspect_ratio: bool = False
    ) -> tuple[float, float]:
        """
        ...
        if match_aspect_ratio:
            ...
            base_width, base_height = cls.get_documented_size_for_aspect_ratio(
    ...
    def scale_coordinate_list(
        cls, coords: list | tuple, viewport_width: int, viewport_height: int
    ) -> list:
        """
