...
class BrowserTool(BaseAnthropicTool):
    ...
    async def _ensure_browser(self) -> None:
        ...
        if self._initialized:
            print(
                "[Browser] Reusing existing browser instance",
                file=sys.stderr,
                flush=True,
            )
            if self._page:
        ...
        if not self._initialized:
            print(
                "[Browser] Initializing browser for first time",
                file=sys.stderr,
                flush=True,
            )
            ...
            if self._browser is None:
                ...
                print(
                    "[Browser] New browser instance created",
                    file=sys.stderr,
                    flush=True,
                )
    ...
    async def _press_key(
        ...
        try:
            ...
            if "+" in key:
                ...
                return ToolResult(
                    output=f"Pressed key combination: {mapped_key}", error=None
                )
    ...
    async def _mouse_down(self, x: int, y: int) -> ToolResult:
        ...
        try:
            ...
            await self._page.mouse.down()
            return ToolResult(
                output=f"Mouse down at ({scaled_x}, {scaled_y})", error=None
            )
    ...
    async def _mouse_up(self, x: int, y: int) -> ToolResult:
        ...
        try:
            ...
            await self._page.mouse.up()
            return ToolResult(
                output=f"Mouse up at ({scaled_x}, {scaled_y})", error=None
            )
    ...
    async def _hover(
        ...
        try:
            ...
            if ref:
                ...
                raise ToolError("Either coordinate or ref is required for hover action")
    ...
    async def __call__(
        ...
        **kwargs,
        ...
        if action == "navigate":
            ...
            if not region:
                raise ToolError("Region (x1, y1, x2, y2) is required for zoom action")
            x1, y1, x2, y2 = region
