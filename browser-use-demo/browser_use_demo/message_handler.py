...
class ResponseProcessor:
    ...
    def _build_tool_result(
        ...
        if result.output:
            ...
            if (
                "__PAGE_EXTRACTED__" in output_text
                or "__TEXT_EXTRACTED__" in output_text
            ):
                if "__FULL_CONTENT__" in output_text:
                    marker_pos = output_text.index("__FULL_CONTENT__")
                    output_text = output_text[
                        marker_pos + len("__FULL_CONTENT__") + 1 :
                    ]

            content_list.append({"type": "text", "text": output_text})
        ...
        if result.error:
            ...
            content_list.append({"type": "text", "text": f"Error: {result.error}"})
...
class MessageBuilder:
    ...
    def add_assistant_message(
        self, messages: list[BetaMessageParam], content: list[BetaContentBlockParam]
    ) -> None:
        """
    ...
    def ensure_message_integrity(self, messages: list[BetaMessageParam]) -> bool:
        """
    ...
    def extract_text_from_message(self, message: BetaMessageParam) -> str | None:
        """
