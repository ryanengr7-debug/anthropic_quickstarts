            ...
            for item in content:
                if isinstance(item, dict):
                    ...
                    processor = processors.get(
                        content_type, extractor.process_default_content
                    )
    ...
    for message in messages:
        cleaned_message = {
            ...
            "content": _format_message_content(
                message.get("content", ""), include_images
            ),
        }
        transcript["conversation"].append(cleaned_message)
...
def main():
    ...
    st.set_page_config(
        page_title="Claude Browser Use Demo", page_icon="🌐", layout="wide"
    )
    ...
    with st.sidebar:
        ...
        if st.session_state.messages:
            ...
            if include_images:
                ...
                zip_data = create_transcript_zip(
                    st.session_state.messages, include_images=True
                )
                ...
                transcript_json = format_transcript_for_download(
                    st.session_state.messages, include_images=False
                )
        ...
        if st.button("Reset Browser", type="secondary"):
            ...
            if (
                st.session_state.event_loop is None
